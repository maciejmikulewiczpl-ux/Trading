...
mkdir -p "$ROOT/logs"
chmod +x "$ROOT/scripts/launch_orb.sh" "$ROOT/scripts/launch_dualmom.sh" "$ROOT/scripts/launch_news_orb.sh" "$ROOT/scripts/launch_lottery_bot.sh" "$ROOT/scripts/launch_lottery_eod.sh"

# ----- 5. systemd units -----
echo "==> Installing systemd units (sudo)"
SYS_DIR="/etc/systemd/system"
for unit in orb.service orb.timer dualmom.service dualmom.timer status.service news-orb.service news-orb.timer lottery-bot.service lottery-bot.timer lottery-eod.service lottery-eod.timer; do
...
sudo systemctl stop orb.timer news-orb.timer lottery-bot.timer lottery-eod.timer 2>/dev/null || true
...
sudo systemctl enable --now orb.timer news-orb.timer lottery-bot.timer lottery-eod.timer
