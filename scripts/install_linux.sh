...
for unit in orb.service orb.timer dualmom.service dualmom.timer status.service news-orb.service news-orb.timer news-hold.service news-hold.timer lottery-bot.service lottery-bot.timer meanrev.service meanrev.timer; do
...
sudo systemctl stop orb.timer news-orb.timer news-hold.timer lottery-bot.timer meanrev.timer 2>/dev/null || true

echo "==> Enabling + starting timers"
# dualmom.timer RETIRED 2026-06-12 (replaced by the lottery experiment). news-orb.timer RETIRED
# 2026-08-07 (same-day news signal was dead; re-structured to news-hold = multi-day catalyst hold
# on the shared hype engine via LOTTERY_* env overrides). Both stay in the repo revivable but are
# NOT enabled here.
sudo systemctl enable --now orb.timer news-hold.timer lottery-bot.timer meanrev.timer
