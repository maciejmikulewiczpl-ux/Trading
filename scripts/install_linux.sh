...
for unit in orb.service orb.timer dualmom.service dualmom.timer status.service news-orb.service news-orb.timer news-hold.service news-hold.timer lottery-bot.service lottery-bot.timer; do
...
sudo systemctl stop orb.timer news-orb.timer news-hold.timer lottery-bot.timer 2>/dev/null || true
...
# NOT enabled here.
sudo systemctl enable --now orb.timer news-hold.timer lottery-bot.timer
