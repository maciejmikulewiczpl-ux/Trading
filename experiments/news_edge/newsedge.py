...
def cmd_log(picks_path: str) -> int:
    ...
    rec = {"date": today, "logged_at": datetime.now(ET).isoformat(timespec="seconds"),
           "picks": [{"symbol": p["symbol"].upper(), "signal": int(p["signal"]),
                      ...
                      "reason": p.get("reason", ""), "sources": p.get("sources", [])} for p in picks]}
...
def cmd_analyze() -> int:
    ...
    for f in sorted(PICKS_DIR.glob("*.json")):
        rec = json.load(open(f))
        for p in rec["picks"]:
            if p.get("ret_945_close") is not None:
                rows.append((p["signal"], p["ret_945_close"], p["confidence"]))
    if not rows:
    ...
    def stats(sel):
        ...
        return f"n={len(r):>3}  avg {sum(r)/len(r):+.2f}%  win {wins/len(r)*100:.0f}%  med {sorted(r)[len(r)//2]:+.2f}%"
    pos = [x for x in rows if x[0] > 0]; neg = [x for x in rows if x[0] < 0]; neu = [x for x in rows if x[0] == 0]
    print(f"=== news-edge forward test: {len(rows)} scored picks over {len(list(PICKS_DIR.glob('*.json')))} days ===")
    print(f"  (+) frontrunners : {stats(pos)}")
    print(f"  ( ) neutral      : {stats(neu)}")
    print(f"  (-) avoid        : {stats(neg)}")
    if pos and neg:
        gap = sum(x[1] for x in pos)/len(pos) - sum(x[1] for x in neg)/len(neg)
        print(f"\n  (+)-minus-(-) separation: {gap:+.2f}%/name  <- the edge, if positive and it grows with n")
    print("\n  Verdict needs dozens of days. One good week is noise; persistence across weeks is signal.")
    return 0
